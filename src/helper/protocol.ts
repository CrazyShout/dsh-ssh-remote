...
export const DSH_RPC_VERSION = '1' as const;
export const DSH_RPC_PROTOCOL = 1 as const;
...
export function parseDshRpcFrame(value: unknown): DshRpcFrame {
  ...
  if (hasMethod) {
    ...
    if (hasResult || hasError) throw new DshRpcProtocolError('RPC request cannot also be a response');
    if (object.params !== undefined) asObject(object.params, 'RPC params');
    if (hasId) assertRpcId(object.id);
    return object as unknown as DshRpcRequest | DshRpcNotification;
  }
...
export function parseServerHello(notification: DshRpcNotification): ServerHello {
  ...
  for (const key of ['system', 'release', 'machine', 'python'] as const) {
  ...
  return {
    ...
    platform: platform as unknown as HelperPlatform,
    capabilities,
    limits,
  };
}
...
export function parseInitializeResult(value: unknown): InitializeResult {
  ...
  for (const key of ['sessionId', 'clientId', 'resumeToken'] as const) {
  ...
  return {
    ...
    session: session as unknown as HelperSession,
    capabilities: asObject(result.capabilities, 'initialize capabilities'),
    limits: asObject(result.limits, 'initialize limits'),
  };
}

export function assertProtocolCompatible(hello: ServerHello): void {
  if (hello.protocol.min > DSH_RPC_PROTOCOL || hello.protocol.max < DSH_RPC_PROTOCOL) {
    throw new DshRpcProtocolError(
      `helper protocol range ${hello.protocol.min}-${hello.protocol.max} does not include ${DSH_RPC_PROTOCOL}`,
    );
  }
}

function parseProtocolRange(value: unknown): HelperProtocolRange {
...
function parseErrorBody(value: unknown): DshRpcErrorBody {
  const error = asObject(value, 'RPC error');
  if (typeof error.code !== 'string' || error.code.length === 0) {
    throw new DshRpcProtocolError('RPC error.code must be a non-empty string');
  }
  if (typeof error.message !== 'string') throw new DshRpcProtocolError('RPC error.message must be a string');
  if (typeof error.retryable !== 'boolean') throw new DshRpcProtocolError('RPC error.retryable must be boolean');
  return error as unknown as DshRpcErrorBody;
}

function assertRpcId(value: unknown): asserts value is RpcId {
