...
export class RemoteFileSystem extends FileSystem {
  ...
  async streamText(target: FsTarget, signal?: AbortSignal): Promise<AsyncIterable<string>> {
    ...
    const mapError = (error: unknown): FsError => this.mapError(error);
    return {
  ...
  async listDir(target: FsTarget, signal?: AbortSignal): Promise<FsDirEntry[]> {
    ...
    return this.sftp(uri, async (sftp) => {
      const entries = await promisify<Array<{ filename: string; attrs: Stats }>>(
        (cb) => sftp.readdir(path, cb),
        signal,
      );
  ...
  private async writeTextUnlocked(
    ...
    await this.writeFile(
      ...
      expected?.kind === 'replaceIfVersion' ? expected.version : undefined,
      expected?.kind === 'replaceIfVersion' && before !== null
  ...
  private async editTextUnlocked(
    target: FsTarget,
    edit: FsEditRequest,
    expected?: { version: FsVersion },
    signal?: AbortSignal,
    ...
    const guarded = { kind: 'replaceIfVersion' as const, version: stat.version };
  ...
  private parseTargetPath(path: string, cwd?: string): SshUri {
    if (path.startsWith('ssh://')) return normalizeSshUri(parseSshUri(path));
   ...
   */
  private async canonicalizePath(uri: SshUri, signal?: AbortSignal): Promise<string> {
  ...
  private async writeFile(
    ...
    await this.sftp(uri, async (sftp) => {
      ...
      const temporary = posix.join(
        posix.dirname(path),
        `.${posix.basename(path)}.dsh-${randomBytes(8).toString('hex')}.tmp`,
      );
      try {
        let existing: Stats | undefined;
        if (!createIfAbsent) {
          try {
            existing = await promisify<Stats>((cb) => sftp.stat(path, cb));
          } catch (error) {
            if (!isCode(error, 'ENOENT')) throw error;
          }
        }
        await promisify<void>((cb) => sftp.writeFile(temporary, data, {
          flag: 'wx',
          mode: 0o600,
        }, cb));
        signal?.throwIfAborted();
        if (createIfAbsent) {
          try {
            ...
            await promisify<void>((cb) => sftp.ext_openssh_hardlink(temporary, path, cb));
          } catch (error) {
            if (isUnsupportedRename(error)) {
              throw new FsError(
                'remote SFTP server does not support atomic hardlink create',
                'FS_IO_ERROR',
            ...
            try {
              destination = await promisify<Stats>((cb) => sftp.lstat(path, cb));
            } catch (statError) {
              if (!isCode(statError, 'ENOENT')) throw this.mapError(statError);
            }
        ...
        if (existing?.mode !== undefined) {
          await promisify<void>((cb) => sftp.chmod(temporary, existing.mode & 0o7777, cb));
        }
        signal?.throwIfAborted();
        if (expectedVersion !== undefined) {
          let current: Stats;
          try {
            current = await promisify<Stats>((cb) => sftp.stat(path, cb));
          } catch (error) {
        ...
        if (expectedContent !== undefined) {
          let current: Buffer;
          try {
            current = await promisify<Buffer>((cb) => sftp.readFile(path, cb));
          } catch (error) {
            if (isCode(error, 'ENOENT')) {
              throw new FsError('file disappeared before publish', 'FS_STALE_VERSION', {
                cause: error as Error,
        ...
        signal?.throwIfAborted();
        try {
          await promisify<void>((cb) => sftp.ext_openssh_rename(temporary, path, cb));
        } catch (error) {
          if (!isUnsupportedRename(error)) throw error;
          throw new FsError(
        ...
        try {
          await promisify<void>((cb) => sftp.unlink(temporary, cb));
        } catch {
  ...
  private async withMutation<T>(key: string, run: () => Promise<T>): Promise<T> {
...
export function createRemoteFileSystemAdapter(
  ...
  Object.defineProperties(adapter, {
    ...
    mutations: { value: new Map<string, Promise<unknown>>() },
  });
  return adapter;
}
...
function statsVersion(st: { mtime: number; size: number; mode?: number; uid?: number; gid?: number }): FsVersion {
  return FsVersion([
    Math.floor(st.mtime),
    st.size,
    st.mode ?? '',
...
function normalizeSshUri(uri: SshUri): SshUri {
...
async function canonicalizeRemotePath(
  ...
  while (true) {
    try {
      const resolved = await promisify<string>((cb) => sftp.realpath(candidate, cb), signal);
      return normalizeRemote(posix.join(resolved, ...missing));
    } catch (error) {
      if (!isCode(error, 'ENOENT') || candidate === '/') throw error;
      let existing: Stats | undefined;
      try {
        existing = await promisify<Stats>((cb) => sftp.lstat(candidate, cb), signal);
      } catch (lstatError) {
        if (!isCode(lstatError, 'ENOENT')) throw lstatError;
      }
      if (existing?.isSymbolicLink()) {
        throw new FsError(
          `refusing unresolved remote symlink: ${candidate}`,
          'FS_SANDBOX_DENIED',
        );
      }
...
function codeOf(e: unknown): string | undefined {
  ...
  if (typeof code === 'number') return sftpStatusToNode(code);
  return typeof code === 'string' ? code : undefined;
}

/** Map a numeric SFTP status code to a Node-style error code string. */
function sftpStatusToNode(code: number): string | undefined {
  switch (code) {
...
function isUnsupportedRename(error: unknown): boolean {
