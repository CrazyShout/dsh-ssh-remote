...
interface HelperStatResult {
  exists: boolean;
  metadata: HelperMetadata | null;
}

interface HelperListResult {
  entries: Array<{
    name: string;
...
interface HelperReadOpenResult {
  handleId: string;
  metadata: HelperMetadata;
}

interface HelperReadNextResult {
  seq: string;
  data: string;
  eof: boolean;
  version?: string;
...
interface HelperWriteResult {
  operation?: 'create' | 'update';
  version: string;
}

interface HelperWriteOpenResult {
  handleId: string;
  seq: string;
...
interface HelperWriteChunkResult {
  seq: string;
  written: number;
  totalBytes: number;
}

type HelperWriteIntent = ReturnType<typeof toHelperWriteIntent>;

type WorkspaceAccess = 'read-only' | 'workspace-write' | 'danger-full-access';
...
export class HelperRemoteFileSystem {
  private readonly workspaces = new Map<string, Promise<WorkspaceOpenResult>>();
  ...
  async resolve(path: string, opts?: { cwd?: string; signal?: AbortSignal }): Promise<FsTarget> {
    ...
    try {
      const result = await scope.client.call<CanonicalizeResult>('fs/canonicalize', {
  ...
  contains(parent: FsTarget, child: FsTarget): boolean {
    ...
    if (left.host !== right.host || left.port !== right.port || left.user !== right.user) return false;
  ...
  async stat(target: FsTarget, signal?: AbortSignal): Promise<FsInfo | undefined> {
    ...
    try {
      const result = await scope.client.call<HelperStatResult>('fs/stat', {
        workspaceId: scope.workspaceId,
        path: scope.path,
        follow: true,
      }, { signal });
  ...
  async streamText(target: FsTarget, signal?: AbortSignal): Promise<AsyncIterable<string>> {
    ...
    const opened = await scope.client.call<HelperReadOpenResult>('fs/readOpen', {
      workspaceId: scope.workspaceId,
      path: scope.path,
      handleId,
    ...
    return {
      async *[Symbol.asyncIterator]() {
        ...
        try {
          for (;;) {
            signal?.throwIfAborted();
            const next = await client.call<HelperReadNextResult>('fs/readNext', {
            ...
            if (bytes.includes(0)) throw new FsError('binary file', 'FS_NOT_TEXT');
            let text: string;
            try {
  ...
  async readBytes(target: FsTarget, signal: AbortSignal | undefined, maxBytes: number): Promise<Uint8Array> {
    ...
    if (bytes.length > maxBytes) throw new FsError(`file exceeds ${maxBytes} bytes`, 'FS_TOO_LARGE');
  ...
  async listDir(target: FsTarget, signal?: AbortSignal): Promise<FsDirEntry[]> {
    ...
    const scope = await this.scope(uri, '/', 'read-only', signal);
    try {
      const result = await scope.client.call<HelperListResult>('fs/list', {
        workspaceId: scope.workspaceId,
        path: scope.path,
        limit: 1_000,
  ...
  async editText(
    ...
    if (oldString.length === 0) throw new FsError('empty oldString', 'FS_AMBIGUOUS_EDIT');
    const matches = allMatches(before, oldString);
    let after: string;
    if (edit.replaceAll) after = before.split(oldString).join(newString);
    else if (matches.length === 0) throw new FsError('oldString not found', 'FS_EDIT_NOT_FOUND');
    else if (matches.length > 1) throw new FsError('oldString matches more than once', 'FS_AMBIGUOUS_EDIT');
    else after = before.slice(0, matches[0]) + newString + before.slice(matches[0] + oldString.length);
    ...
    try {
      const result = await this.write(
        ...
        { kind: 'replace-if-version', version: read.version },
        signal,
      );
  ...
  private async read(target: FsTarget, maxBytes: number, signal?: AbortSignal): Promise<HelperReadResult> {
    const { uri } = this.split(target);
    const scope = await this.scope(uri, '/', 'read-only', signal);
    const handleId = randomUUID();
    ...
    try {
      ...
      for (;;) {
        ...
        total += chunk.length;
        if (total > maxBytes) throw new FsError(`file exceeds ${maxBytes} bytes`, 'FS_TOO_LARGE');
  ...
  private async readBeforeText(target: FsTarget, signal?: AbortSignal): Promise<string | null> {
    try {
  ...
  private async write(
    scope: HelperScope,
    data: Buffer,
    intent: HelperWriteIntent,
    signal?: AbortSignal,
  ): Promise<HelperWriteResult> {
    const handleId = randomUUID();
    let opened: HelperWriteOpenResult | undefined;
    let committed = false;
    try {
      signal?.throwIfAborted();
      opened = await scope.client.call<HelperWriteOpenResult>('fs/writeOpen', {
        workspaceId: scope.workspaceId,
        path: scope.path,
        intent,
        handleId,
      ...
      for (let offset = 0; offset < data.length; offset += chunkBytes) {
        ...
        const response = await scope.client.call<HelperWriteChunkResult>('fs/writeChunk', {
          handleId: opened.handleId,
          afterSeq: cursor,
      ...
      signal?.throwIfAborted();
      const result = await scope.client.call<HelperWriteResult>('fs/writeCommit', {
        handleId: opened.handleId,
  ...
  private async mutationScope(
    ...
    if (mode === 'read-only') throw new FsError('remote file access denied under read-only mode', 'FS_SANDBOX_DENIED');
  ...
  private async scope(
    ...
    if (cacheable) {
      ...
      for (const existingKey of this.workspaces.keys()) {
        if (existingKey !== key && existingKey.endsWith(authoritySuffix)) this.workspaces.delete(existingKey);
      }
    }
    ...
    if (opened === undefined) {
      ...
      signal?.throwIfAborted();
      opened = client.call<WorkspaceOpenResult>('workspace/open', {
        path: canonicalRoot,
        access,
        workspaceId,
        operationId: workspaceId,
      }, { timeoutMs: 20_000, mutation: true });
      if (cacheable) this.workspaces.set(key, opened);
      void opened.catch(() => {
        if (this.workspaces.get(key) === opened) this.workspaces.delete(key);
      });
    }
  ...
  private parseTargetPath(path: string, cwd?: string): SshUri {
    if (path.startsWith('ssh://')) return normalizeUri(parseSshUri(path));
  ...
  private target(uri: SshUri): FsTarget {
    const normalized = normalizeUri(uri);
    return {
...
function normalizeUri(uri: SshUri): SshUri {
...
function toHelperWriteIntent(intent: FsWriteIntent | undefined): {
  kind: 'overwrite' | 'create-if-absent' | 'replace-if-version';
  version?: string;
} {
  if (intent === undefined) return { kind: 'overwrite' };
  if (intent.kind === 'createIfAbsent') return { kind: 'create-if-absent' };
  return { kind: 'replace-if-version', version: String(intent.version) };
}

function toFsInfo(stat: HelperMetadata): FsInfo {
  if (stat.version === undefined) throw new FsError('helper stat omitted version', 'FS_IO_ERROR');
  return {
...
function toFsPathInfo(stat: HelperMetadata): FsPathInfo {
  if (stat.version === undefined) throw new FsError('helper lstat omitted version', 'FS_IO_ERROR');
  return {
    type: stat.type,
    size: stat.size,
    version: FsVersion(stat.version),
...
function mapHelperFsError(error: unknown): FsError {
  if (error instanceof FsError) return error;
  const code = error instanceof RemoteHelperRpcError ? error.code : '';
  const mapped =
    code === 'E_NOT_FOUND' || code === 'ENOENT' ? 'FS_NOT_FOUND' :
  ...
  return new FsError(`remote helper fs: ${message}`, mapped, { cause: error as Error });
}

function decodeText(buffer: Buffer): string {
  if (buffer.includes(0)) throw new FsError('binary file', 'FS_NOT_TEXT');
  try {
