...
export function parseSshUri(uri) {
    ...
    if (hostport.startsWith('[')) {
        ...
        if (bracket === -1)
            throw new Error(`invalid ssh uri (unterminated IPv6 host): ${uri}`);
        host = hostport.slice(1, bracket);
        const suffix = hostport.slice(bracket + 1);
        if (suffix) {
            if (!suffix.startsWith(':'))
                throw new Error(`invalid ssh uri authority: ${uri}`);
    ...
    if (host.startsWith('-') || /[\s\0]/u.test(host))
        throw new Error(`invalid ssh uri host: ${uri}`);
...
function parseSshPort(value, uri) {
    if (!/^\d+$/u.test(value))
