...
export const MODERN_SSH_CAPABILITIES = { sessionTypeSupported: true };
const capabilityCache = new Map();
/** Probe supported transport options without connecting to a host. */
export async function detectSshCapabilities(sshBinary = 'ssh', probeOptions = probeSystemSshOptions) {
...
export function resetSshCapabilityCache() {
    capabilityCache.clear();
}
/** `-G` expands isolated options and never opens a network connection. */
function probeSystemSshOptions(sshBinary) {
...
export function buildSystemSshArgs(alias, remoteCommand, capabilities = MODERN_SSH_CAPABILITIES) {
    ...
    args.push('-o', 'ConnectTimeout=15', '-o', 'ServerAliveInterval=15', '-o', 'ServerAliveCountMax=3', '--', alias, remoteCommand);
    return args;
}
/**
