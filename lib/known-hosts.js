 ...
 */
export async function loadOpenSshHostTrust(config, fallbackHost, fallbackPort, runner = runSshKeygen) {
    ...
    for (const file of files) {
        ...
        if (result.code === 1)
            continue; // no matching host in this file
        if (result.code !== 0) {
        ...
        for (const key of parsed.trustedKeys)
            trustedKeys.add(key);
        for (const key of parsed.revokedKeys)
            revokedKeys.add(key);
...
export function parseSshKeygenOutput(output) {
    const trustedKeys = new Set();
    const revokedKeys = new Set();
    let certificateAuthorities = 0;
...
export function createHostVerifier(trust) {
    return (key) => {
        const encoded = key.toString('base64');
        if (trust.revokedKeys.has(encoded))
            return false;
        if (trust.policy === 'no')
            return true;
 ...
 */
export function assertHostTrustReady(trust, alias) {
    if (trust.policy === 'no' || trust.trustedKeys.size > 0)
        return;
    const authorityDetail = trust.certificateAuthorities > 0
        ...
        : '';
    throw new Error(`SSH host key for ${trust.lookup} is not trusted for ${alias}. Run "ssh ${alias}" once and verify the fingerprint, then retry.${authorityDetail}`);
}
export function normalizeStrictHostKeyChecking(value) {
...
function runSshKeygen(lookup, file) {
    return new Promise((resolveResult) => {
        execFile('ssh-keygen', ['-F', lookup, '-f', file], { encoding: 'utf8', timeout: 5_000, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
